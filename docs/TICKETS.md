...
| ET020   | Unit tests, integration tests, Playwright E2E, FRS traceability matrix, CI quality gates, release readiness  | All  | Quality  | PR open (#22) | [#22](https://github.com/balwant-rathore/ExpenseTracker/pull/22) | et020-quality-release |
| ET021   | Fix BR-10 audit-timestamp storage (persist via `ICompanyClock`, not raw `UtcNow`); clarify/fix Currency defaulting behavior for expense creation; hide the `ExpenseListPage` "New expense" button for roles (Finance, ComplianceOfficer) that `/expenses/new`'s route guard already blocks. All three found during the ET020 traceability audit and deliberately not fixed there (verification-only scope) — see `docs/TRACEABILITY.md` flags. | 4, 8, 11 | Expenses | Planned | Found during ET020 audit |  |
