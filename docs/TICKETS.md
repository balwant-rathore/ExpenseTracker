...
| ET017   | Expense management UI (create, edit, cancel, upload receipt, list, detail)   | 4  | Frontend       | PR open (#17) | [#17](https://github.com/balwant-rathore/ExpenseTracker/pull/17) | et017-expense-ui |
| ET018   | Manager, Compliance and Finance review screens. Also revisit ET017 finding: `EditExpensePage` renders the edit form for a fetched expense before any client-side ownership check — only the Edit-link visibility on the detail page and the backend's authoritative 403 on `PUT` currently gate non-owner access; add a route-level ownership guard so a non-owner (e.g. a Manager viewing a direct report's `Submitted` expense) can't navigate directly to `/expenses/{id}/edit` and see it rendered as editable | 5, 7  | Frontend  | PR open (#18) | [#18](https://github.com/balwant-rathore/ExpenseTracker/pull/18) | et018-expense-review |
