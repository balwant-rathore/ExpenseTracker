# expense-notifications Specification

## Purpose
Post-commit HTML notification logging for expense workflow status changes. Whenever an
expense transitions to `Submitted`, `Approved`, `Rejected`, `ComplianceApproved`,
`ComplianceRejected`, or `Reimbursed`, the system resolves the correct recipients (the
expense owner, their reporting manager, and/or all active Finance-role employees,
depending on the event), renders an event-specific subject/body, and appends a
well-formed entry to an append-only HTML log file — without ever blocking or rolling
back the underlying workflow transaction on failure (FRS §9, SDS §7).
